    ...
    const updateData = () => {
        ...
        if (address) {
            contractService.getPionV1Balance(address).then(balance => {
                let fixedBalance = +(BigNumber(balance).toFixed(9))
                setWalletBalance(fixedBalance)
            })
    ...
    return (
        <div className="v2">
            ...
            <div className="v2__box">
                ...
                {isApproved ? <button className="btn btn--big" onClick={onSwap} disabled={!amount || errorCode || amount > walletBalance || (BigNumber(amount).toFixed().split('.')[1] && BigNumber(amount).toFixed().split('.')[1].length > 9)
                }>SWAP</button> :
